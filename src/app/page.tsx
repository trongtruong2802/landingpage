...
export default function HomePage() {
  return (
    <main className="relative overflow-hidden bg-transparent">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[34rem] bg-[radial-gradient(circle_at_top,_rgba(232,210,156,0.3),_transparent_52%)]" />
      <div className="pointer-events-none absolute left-0 top-[42rem] h-72 w-72 rounded-full bg-[radial-gradient(circle,_rgba(203,176,112,0.14),_transparent_70%)] blur-3xl" />
      <div className="pointer-events-none absolute right-0 top-[92rem] h-80 w-80 rounded-full bg-[radial-gradient(circle,_rgba(255,255,255,0.28),_transparent_68%)] blur-3xl" />

      <header className="sticky top-0 z-30 border-b border-[color:var(--border)] bg-[rgba(255,252,247,0.82)] backdrop-blur-xl">
        <Container className="flex flex-col gap-2 py-3 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:py-4">
          <div>
            <p className="text-[0.65rem] uppercase tracking-[0.45em] text-[color:var(--primary)]">
              Wedding Day
            </p>
          ...
          >
            {navigationItems.map((item) => (
              <a
                className="min-h-11 whitespace-nowrap rounded-full border border-[color:var(--border)] bg-white/70 px-3 py-2 text-[0.7rem] uppercase tracking-[0.18em] text-[color:var(--muted)] transition hover:border-[color:var(--primary)] hover:text-[color:var(--foreground)] sm:px-4 sm:text-xs sm:tracking-[0.22em]"
                href={item.href}
                key={item.href}
              >
                {item.label}
